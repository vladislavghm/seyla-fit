...
# Используем минимальный лимит памяти и ОБЯЗАТЕЛЬНО 1 воркер
# Это заставит Next.js использовать swap, но сборка пройдет
export NODE_OPTIONS="--max-old-space-size=600"
# Ограничиваем количество воркеров до 1 (критично для малой памяти!)
# Next.js 15 использует эту переменную окружения
export NEXT_PRIVATE_WORKERS=1
# Отключаем линтер для экономии памяти
pnpm next build --no-lint 2>&1 || {
    ...
    echo ""
    echo -e "${YELLOW}   Рекомендуется загрузить готовую сборку с локального компьютера:${NC}"
    echo -e "${YELLOW}   scp -r .next vladhoyl@155.212.188.120:~/seyla-fit/${NC}"
    exit 1
}
